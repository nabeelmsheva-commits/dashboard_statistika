...
# ─────────────────────────────────────────────
# CUSTOM CSS (AURORA GLASS GEN Z THEME)
# ─────────────────────────────────────────────
st.markdown("""
<style>
...
.orb1 { width: 550px; height: 550px; background: #a78bfa; top: -120px; left: -100px; animation: float1 22s infinite alternate ease-in-out; }
.orb2 { width: 650px; height: 650px; background: #22d3ee; bottom: -180px; right: -150px; animation: float2 26s infinite alternate ease-in-out; }
.orb3 { width: 450px; height: 450px; background: #f472b6; top: 45%; left: 40%; animation: float3 20s infinite alternate ease-in-out; }

@keyframes float1 { 0% { transform: translate(0, 0) scale(1); } 100% { transform: translate(220px, 180px) scale(1.15); } }
@keyframes float2 { 0% { transform: translate(0, 0) scale(1); } 100% { transform: translate(-280px, -180px) scale(1.25); } }
@keyframes float3 { 0% { transform: translate(0, 0) scale(1); } 100% { transform: translate(-180px, 220px) scale(0.9); } }
...
.main .block-container { padding-top: 1.5rem; padding-bottom: 2rem; z-index: 1; position: relative; }

/* ─── SIDEBAR (ALWAYS OPEN + FIXED Z-INDEX) ─── */
...
[data-testid="stSidebar"] > div { z-index: 1001 !important; position: relative !important; }
...
button[data-testid="baseButton-header"], [data-testid="collapsedControl"] { display: none !important; }
...
[data-testid="metric-container"] [data-testid="stMetricDelta"] { font-weight: 700 !important; }
...
.aurora-subtitle { color: #94a3b8; font-size: 1.05rem; font-weight: 400; margin-top: 8px; }
.section-title {
    color: #f1f5f9;
    font-size: 1.2rem;
    font-weight: 700;
...
[data-testid="stSidebar"] .stSelectbox > div > div { z-index: 1002 !important; position: relative !important; }
...
hr {
    border: none;
    height: 1px;
    background: linear-gradient(90deg, transparent, rgba(167, 139, 250, 0.3), rgba(34, 211, 238, 0.3), transparent);
    margin: 32px 0;
}
...
::-webkit-scrollbar-thumb { background: linear-gradient(180deg, #a78bfa, #22d3ee); border-radius: 10px; border: 2px solid rgba(15, 23, 42, 0.5); }
::-webkit-scrollbar-thumb:hover { background: linear-gradient(180deg, #c4b5fd, #67e8f9); }

@keyframes fadeInUp { from { opacity: 0; transform: translateY(24px); } to { opacity: 1; transform: translateY(0); } }
.element-container, .stPlotlyChart, [data-testid="stMetric"] { animation: fadeInUp 0.6s ease-out forwards; }
...
WARNA_UTAMA = ["#a78bfa", "#22d3ee", "#f472b6", "#34d399", "#fbbf24", "#fb923c"]
BG_PLOT = "rgba(30, 41, 59, 0.35)"
...
with st.sidebar:
    ...
    st.markdown("✨ **Gen Z Finance Dashboard** · 2026")
...
<div style="padding: 20px 0 10px 0;">
    <h1 class="aurora-title">💰 Dashboard Keuangan Mahasiswa</h1>
    <p class="aurora-subtitle">
        Analisis pola pengeluaran & perilaku keuangan mahasiswa Sains Data dengan vibe Gen Z ✨
    </p>
</div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    with col2:
        ...
        fig2 = px.bar(uang_cnt, x="Uang Saku", y="Jumlah", color="Jumlah", color_continuous_scale=[[0, "#6366f1"], [0.5, "#a78bfa"], [1, "#22d3ee"]], text="Jumlah")
...
with tab2:
    ...
    with col2:
        ...
        fig2 = px.bar(faktor_cnt, y="Faktor_short", x="Jumlah", orientation="h", color="Jumlah", color_continuous_scale=[[0, "#6366f1"], [0.5, "#f472b6"], [1, "#fbbf24"]], text="Jumlah")
        fig2.update_traces(textposition="outside", textfont_color=FONT_COLOR)
        fig2.update_coloraxes(showscale=False)
...
with tab3:
    ...
    with col1:
        ...
        fig = px.pie(kh_cnt, names="Status", values="Jumlah", hole=0.55, color="Status", color_discrete_map={"Ya": "#f43f5e", "Tidak": "#34d399"})
        fig.update_traces(textfont_size=13, textinfo="percent+label")
        style_fig(fig)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col2:
        ...
        fig2 = px.pie(bd_cnt, names="Status", values="Jumlah", hole=0.55, color="Status", color_discrete_map={"Ya": "#a78bfa", "Tidak": "#fbbf24"})
...
with tab4:
    ...
    with col_m:
        fig_f = px.bar(freq_df, x="Kategori", y="Frekuensi", color="Frekuensi", color_continuous_scale=[[0, "#6366f1"], [0.5, "#a78bfa"], [1, "#22d3ee"]], text="Persentase")
    ...
    if n >= 5:
        ...
        fig_heat = go.Figure(data=go.Heatmap(
            ...
            colorscale=[[0, "#1e1b4b"], [0.3, "#6366f1"], [0.6, "#a78bfa"], [1, "#22d3ee"]],
            zmin=0, zmax=1,
...
with tab5:
    ...
    if st.button("🚀 Jalankan Simulasi Monte Carlo", type="primary"):
        with st.spinner("Menjalankan 10.000 iterasi simulasi..."):
            ...
            fig_sim = px.histogram(df_sim, x="Sisa Uang", nbins=50, color_discrete_sequence=["#a78bfa"], marginal="box", opacity=0.85)
